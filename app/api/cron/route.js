...
export async function POST(req, res) {
  ...
  const tempDir = '/tmp'  // Ensure you're using the /tmp directory
  ...
  console.log('FFmpeg Path:', ffmpegPath)  // Log the ffmpeg path
  ...
  try {
    ...
    try {
      ...
      console.log(`Audio file downloaded: ${audioFilePath} (${audioBuffer.length} bytes)`)
    } catch (audioError) {
    ...
    try {
      ...
      console.log(`Image file downloaded: ${imageFilePath} (${imageBuffer.length} bytes)`)
    } catch (imageError) {
